...
async function inicializarContratos() {
    try {
        const [resC, resCl] = await Promise.all([
            fetch(`${CONT_API}/cotizaciones`),
            fetch(`${CONT_API}/clientes`)
        ]);
        _cotizaciones = await resC.json();
        _clientes     = await resCl.json();

        const sel = document.getElementById('cotizacion-select');
        sel.innerHTML = '<option value="">-- Selecciona una cotización --</option>';

        if (!Array.isArray(_cotizaciones) || _cotizaciones.length === 0) {
            sel.innerHTML += '<option value="">No hay cotizaciones registradas</option>';
            return;
        }

        _cotizaciones.forEach(c => {
            const cliente = _clientes.find(cl => cl.id_clientes === c.CLIENTES_id_clientes);
            const nombre  = cliente ? cliente.NombreCompleto : `Cliente #${c.CLIENTES_id_clientes}`;
            const fecha   = c.fecha_emision ? new Date(c.fecha_emision).toLocaleDateString('es-MX') : '—';
            const monto   = parseFloat(c.monto_estimado || 0).toLocaleString();
            sel.innerHTML += `<option value="${c.id_cotizaciones}">
                ${nombre} — $${monto} — ${fecha}
            </option>`;
        });
    } catch (e) {
...
function cargarDesdeCotizacion() {
    const cotId = document.getElementById('cotizacion-select').value;
    if (!cotId) return;

    const cot     = _cotizaciones.find(c => c.id_cotizaciones === parseInt(cotId));
    if (!cot) return;

    const cliente = _clientes.find(cl => cl.id_clientes === cot.CLIENTES_id_clientes);
    ...
    document.getElementById('contrato-monto').value = cot.monto_estimado || '';

    // Extraer tipo de evento de los detalles
    if (cot.detalles) {
        const tipoMatch = cot.detalles.split('|')[0].trim();
        const select    = document.getElementById('tipo-evento');
        for (let opt of select.options) {
            if (tipoMatch.includes(opt.value)) {
...
async function guardarYImprimir() {
    ...
    try {
        ...
        if (fechaDuplicada) {
            const fechaFmt = new Date(fecha).toLocaleDateString('es-MX');
            status.style.color = 'red';
            status.textContent = `⚠️ Ya existe un evento agendado para el ${fechaFmt}.`;
            alert(`⚠️ Ya existe un evento para el ${fechaFmt}. Elige otra fecha.`);
            return;
        }

        status.textContent = 'Guardando...';

        let clienteId, cotizacionId;

        if (cotId) {
            // Usar cotización existente
            const cot = _cotizaciones.find(c => c.id_cotizaciones === parseInt(cotId));
            clienteId    = cot.CLIENTES_id_clientes;
            cotizacionId = parseInt(cotId);
        } else {
            // Crear cliente nuevo
        ...
        const resRes = await fetch(`${CONT_API}/reservaciones`, {
            ...
            body: JSON.stringify({
                ...
                COTIZACIONES_id_cotizaciones:      cotizacionId,
...
async function cancelarEvento() {
    ...
    try {
        ...
        if (activas.length === 0) { alert('No hay eventos activos para cancelar.'); return; }
        ...
        const motivo = prompt(`¿Motivo de cancelación del evento de ${nombre} el ${fecha}?`);
        if (!motivo) return;
        ...
        if (!resCan.ok) { alert('Error al cancelar'); return; }
...
function mostrarContratoCancelacion(nombre, fecha, tipo, motivo) {
    ...
    const texto = `
        ...
        <br>
        <p>En la ciudad de Saltillo, Coahuila, a fecha <b>${fechaHoy}</b>, se hace constar la cancelación del contrato celebrado entre <b>SALÓN MUNDO MÁGICO</b> y el C. <b>${nombre}</b>.</p>
        <br>
        <p><b>EVENTO CANCELADO:</b> ${tipo} programado para el día <b>${fecha}</b>.</p>
        <br>
        <p><b>MOTIVO:</b> ${motivo}</p>
        <br>
        <p><b>PRIMERA:</b> Ambas partes acuerdan de manera voluntaria la cancelación del evento.</p>
        <br>
        <p><b>SEGUNDA:</b> El anticipo entregado <b>NO será reembolsable</b>.</p>
        <br>
        <p><b>TERCERA:</b> Con la firma del presente, ambas partes dan por terminada toda obligación.</p>
        <br><br><br>
        <div style="display:flex; justify-content: space-around; margin-top: 40px;">
